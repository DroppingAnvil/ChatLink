...
/* ChatLink patch: _WIN32 -> _MSC_VER. MinGW-w64 defines _WIN32 but is GCC,
   so the original test both used MSVC-only __pragma and redefined
   gettimeofday, which MinGW already declares in <sys/time.h>. */
#ifdef _MSC_VER
#include <sys/timeb.h>
#include <sys/types.h>
#include <winsock2.h>
...
static bool readPacket(usb_device_t *handle, NNSEMessage *message, int maxlen)
{
	...
	int r = usb_bulk(handle, NSP_EP_IN, reinterpret_cast<unsigned char*>(message), maxlen, &transferred, 60000);
	...
	{
		r = usb_bulk(handle, NSP_EP_IN, data, remainingLength, &transferred, 1000);
		if(r < 0)
			return false;
...
static bool writePacket(usb_device_t *handle, NNSEMessage *message)
{
	...
	int transferred = 0;
	int r = usb_bulk(handle, NSP_EP_OUT, reinterpret_cast<unsigned char*>(message), length, &transferred, 1000);
	if(r < 0
		|| length != transferred)
		return false;
...
template <typename T> bool sendMessage(usb_device_t *handle, T &message)
{
